...
class GPTDialog(QDialog):
    ...
    def __init__(self, gui, icon, do_user_config):
        ...
        if not self.token:
            ...
            self.l.addWidget(QLabel('Enter a query and press submit.'))
            self.prompt = QLineEdit()
            self.l.addWidget(self.prompt)
            self.submit = QPushButton('Submit', self)
            self.l.addWidget(self.submit)
    ...
    def query(self):
        data = json.loads(self.exec_query().decode("utf-8"))
    ...
    def exec_query(self):
        engine = subprocess.Popen(["python3", "-", 
            ...
            "--match-count", "10",
            "--prompt", self.prompt.text()], stdout = subprocess.PIPE, stdin = subprocess.PIPE)
        return engine.communicate(input = get_resources("engine.py"))[0]
